...
class Network_Multi_Path_Infer(nn.Module):
    ...
    def build_arm_ffm_head(self):
        ...
        self.ffm = FeatureFusion(self.num_filters(8, self._stem_head_width[1]) * self._branch, self.num_filters(8, self._stem_head_width[1]) * self._branch, reduction=1, Fch=self._Fch, scale=8,
                                 branch=self._branch, norm_layer=BatchNorm2d)
    ...
    def get_branch_groups_cells(self, ops, paths, downs, widths, lasts):
        ...
        for l in range(layers):
            ...
            for group in branch_groups:
                ...
                if len(group) >= 2: assert ops[group[0]][l] == ops[group[1]][l] and paths[group[0]][l + 1] == paths[group[1]][l + 1] and downs[group[0]][l] == downs[group[1]][l] and widths[group[0]][
                    l] == widths[group[1]][l]
                if len(group) == 3: assert ops[group[1]][l] == ops[group[2]][l] and paths[group[1]][l + 1] == paths[group[2]][l + 1] and downs[group[1]][l] == downs[group[2]][l] and widths[group[1]][
                    l] == widths[group[2]][l]
                op = ops[group[0]][l]
